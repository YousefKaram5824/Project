...
SearchManager::SearchManager(QMap<int, Court> &courts)
    : courtMap(courts)
{}
...
QList<Court> SearchManager::filterCourts(const QString &location,
                                         const QDate &date,
                                         const QTime &time) const
{
    ...
    for (const Court &court : courtMap) {
        if (court.location.trimmed() == trimmedLocation && court.date == date && court.time == time
            && !court.isBooked) {
            result.append(court);
        }
    }
...
Court SearchManager::findClosestAvailable(const QString &location,
                                          const QDate &date,
                                          const QTime &time) const
{
    Court closest;
